...
export default function TabBar({
  ...
  tabs: Tab[]; active: string|undefined;
  onActivate: (path: string)=>void; onClose: (path:string)=>void; onCopyUrl: (path: string)=>void; onRefresh: (path: string)=>void
}) {
  const [menu, setMenu] = useState<{x:number;y:number;tab:Tab}|null>(null)

  useEffect(()=>{
    function onKey(e: KeyboardEvent){
      if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase()==='q' && active) { e.preventDefault(); onClose(active) }
    }
  ...
  function itemsFor(t: Tab): CtxItem[] {
    return [
      { label:'Refresh', onClick:()=>onRefresh(t.path) },                 // NEW
      { label:'Copy absolute location', onClick:()=>onCopyUrl(t.path) },
      ...
      {tabs.map(t => (
        <div key={t.path}
             ...
             style={{
               ...
               background: active===t.path ? '#0f172a' : '#0f172a',
               borderRadius:6, cursor:'pointer', whiteSpace:'nowrap', userSelect:'none'
             }}>
          <span title={t.path}>{t.name}{t.dirty?'*':''}</span>
          <button onClick={(e)=>{ e.stopPropagation(); onClose(t.path) }}
                  title="Close (Ctrl+Q)"
