...
// node_modules/@codemirror/highlight/node_modules/@codemirror/state/dist/index.js
var DefaultSplit = /\r\n?|\n/;
  ...
  function inner(ext, prec3) {
    ...
    if (Array.isArray(ext)) {
      ...
      if (ext.facet.extensions)
        inner(ext.facet.extensions, prec3);
    } else {
...
var ChangeSet2 = class _ChangeSet extends ChangeDesc2 {
  ...
  */
  get desc() {
    return new ChangeDesc2(this.sections);
  }
  /**
  @internal
  */
  filter(ranges) {
    ...
    return {
      changes: new _ChangeSet(resultSections, resultInserted),
      filtered: new ChangeDesc2(filteredSections)
    };
  }
  /**
  Serialize this change set to a JSON-representable value.
  ...
  static of(changes, length, lineSep) {
    ...
    function process(spec) {
      if (Array.isArray(spec)) {
        ...
        if (from > to || from < 0 || to > length)
          throw new RangeError(`Invalid change range ${from} to ${to} (in doc of length ${length})`);
        let insText = !insert2 ? Text.empty : typeof insert2 == "string" ? Text.of(insert2.split(lineSep || DefaultSplit2)) : insert2;
        let insLen = insText.length;
        if (from == to && insLen == 0)
          return;
        if (from < pos)
...
function mapSet2(setA, setB, before, mkSet = false) {
  ...
  for (let posA = 0, posB = 0; ; ) {
    if (a.ins == -1) {
      ...
      addSection2(sections, len, a.ins);
      if (insert2)
        addInsert2(insert2, sections, a.text);
      posA = end;
      a.next();
    } else if (a.done && b.done) {
      return insert2 ? new ChangeSet2(sections, insert2) : new ChangeDesc2(sections);
    } else {
      throw new Error("Mismatched change set lengths");
    }
  }
...
var EditorSelection2 = class _EditorSelection {
  ...
  */
  static create(ranges, mainIndex = 0) {
    ...
    for (let pos = 0, i = 0; i < ranges.length; i++) {
      let range = ranges[i];
      if (range.empty ? range.from <= pos : range.from < pos)
        return normalized2(ranges.slice(), mainIndex);
      pos = range.to;
    }
    return new _EditorSelection(ranges, mainIndex);
  }
  ...
  */
  static cursor(pos, assoc = 0, bidiLevel, goalColumn) {
    return new SelectionRange2(pos, pos, (assoc == 0 ? 0 : assoc < 0 ? 4 : 8) | (bidiLevel == null ? 3 : Math.min(2, bidiLevel)) | (goalColumn !== null && goalColumn !== void 0 ? goalColumn : 33554431) << 5);
  }
  /**
  Create a selection range.
  */
  static range(anchor, head, goalColumn) {
    let goal = (goalColumn !== null && goalColumn !== void 0 ? goalColumn : 33554431) << 5;
    return head < anchor ? new SelectionRange2(
      head,
      anchor,
      16 | goal | 8
      /* AssocAfter */
    ) : new SelectionRange2(anchor, head, goal | (head > anchor ? 4 : 0));
  }
};
function normalized2(ranges, mainIndex = 0) {
  let main = ranges[mainIndex];
  ranges.sort((a, b) => a.from - b.from);
  mainIndex = ranges.indexOf(main);
  for (let i = 1; i < ranges.length; i++) {
    ...
    if (range.empty ? range.from <= prev.to : range.from < prev.to) {
      ...
      if (i <= mainIndex)
        mainIndex--;
      ranges.splice(--i, 2, range.anchor > range.head ? EditorSelection2.range(to, from) : EditorSelection2.range(from, to));
    }
  }
  return new EditorSelection2(ranges, mainIndex);
}
...
var FacetProvider2 = class {
  ...
  dynamicSlot(addresses) {
    ...
    return {
      ...
      update(state, tr) {
        if (depDoc && tr.docChanged || depSel && (tr.docChanged || tr.selection) || depAddrs.some((addr) => (ensureAddr2(state, addr) & 1) > 0)) {
          let newVal = getter(state);
          if (multi ? !compareArray2(newVal, state.values[idx], compare2) : !compare2(newVal, state.values[idx])) {
            state.values[idx] = newVal;
            return 1;
          }
        }
      ...
      reconfigure(state, oldState) {
        ...
        if (oldAddr != null) {
          ...
          if (multi ? compareArray2(newVal, oldVal, compare2) : compare2(newVal, oldVal)) {
            state.values[idx] = oldVal;
            return 0;
          }
        }
...
function compareArray2(a, b, compare2) {
  if (a.length != b.length)
    return false;
  for (let i = 0; i < a.length; i++)
    if (!compare2(a[i], b[i]))
...
function dynamicFacetSlot2(addresses, facet, providers) {
  ...
  return {
    ...
    update(state, tr) {
      if (!dynamic.some(
        (dynAddr) => ensureAddr2(state, dynAddr) & 1
        /* Changed */
      ))
        return 0;
      let value = get(state);
...
var StateField2 = class _StateField {
  ...
  */
  static define(config) {
    let field = new _StateField(nextID2++, config.create, config.update, config.compare || ((a, b) => a === b), config);
    if (config.provide)
      field.provides = config.provide(field);
    return field;
  }
...
var Prec2 = {
  ...
  */
  ...
  Backwards-compatible synonym for `Prec.lowest`.
  */
  fallback: prec2(Prec_2.lowest),
  /**
  Backwards-compatible synonym for `Prec.high`.
  */
  extend: prec2(Prec_2.high),
  /**
  Backwards-compatible synonym for `Prec.highest`.
  */
  override: prec2(Prec_2.highest)
};
...
function filterTransaction2(tr) {
  ...
  if (result !== true) {
    ...
    tr = new Transaction2(state, changes, tr.selection && tr.selection.map(back), StateEffect2.mapEffects(tr.effects, back), tr.annotations, tr.scrollIntoView);
  }
...
function extendTransaction2(tr) {
  ...
  for (let i = extenders.length - 1; i >= 0; i--) {
    let extension = extenders[i](tr);
    if (extension && Object.keys(extension).length)
      spec = mergeTransaction2(tr, resolveTransactionInner2(state, extension, tr.changes.newLength), true);
  }
...
var EditorState2 = class _EditorState {
  ...
  */
  toText(string2) {
    return Text.of(string2.split(this.facet(_EditorState.lineSeparator) || DefaultSplit2));
  }
  /**
  Return the given range of the document as a string.
  */
  ...
  */
  static create(config = {}) {
    let configuration = Configuration2.resolve(config.extensions || [], /* @__PURE__ */ new Map());
    let doc2 = config.doc instanceof Text ? config.doc : Text.of((config.doc || "").split(configuration.staticFacet(_EditorState.lineSeparator) || DefaultSplit2));
    let selection = !config.selection ? EditorSelection2.single(0) : config.selection instanceof EditorSelection2 ? config.selection : EditorSelection2.single(config.selection.anchor, config.selection.head);
...
RangeValue.prototype.mapMode = MapMode2.TrackDel;
var Range2 = class {
  /**
  @internal
  */
...
// node_modules/@codemirror/highlight/node_modules/@codemirror/view/dist/index.js
...
var ViewUpdate = class {
  ...
  @internal
  */
  constructor(view, state, transactions = none3) {
    this.view = view;
    this.state = state;
...
var BlockInfo = class _BlockInfo {
  ...
  */
  moveY(offset) {
    return !offset ? this : new _BlockInfo(this.from, this.length, this.top + offset, this.height, Array.isArray(this.type) ? this.type.map((b) => b.moveY(offset)) : this.type);
  }
};
...
// node_modules/@codemirror/highlight/node_modules/@codemirror/language/dist/index.js
var _a;
...
var Modifier = class _Modifier {
  ...
  static get(base2, mods) {
    if (!mods.length)
      return base2;
    let exists = mods[0].instances.find((t2) => t2.base == base2 && sameArray3(mods, t2.modified));
    if (exists)
      return exists;
